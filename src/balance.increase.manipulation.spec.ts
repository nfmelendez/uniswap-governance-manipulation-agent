...
describe("Uniswap governance manipulation agent", () => {
  ...
  const mockContractUtils = {
    getProposalPriorVotes : jest.fn(),
  } as any

  beforeEach(() => {
    mockTxEvent.filterLog.mockReset();
    mockContractUtils.getProposalPriorVotes.mockReset();
  });

  describe("handleTransaction", () => {
    it ("returns empty findings if the voter didn't have a significant votes change previous the proposal", async () => {
  
      const handleTransaction = balanceIncreaseManipulationAgent.provideHandleTransaction(mockContractUtils)
      ...
      mockContractUtils.getProposalPriorVotes.mockReturnValueOnce(new BigNumber(votesPriorProposal))

      const findings = await handleTransaction(mockTxEvent)

      expect(mockTxEvent.filterLog.mock.calls.length).toBe(1)
      expect(mockContractUtils.getProposalPriorVotes.mock.calls.length).toBe(1)
